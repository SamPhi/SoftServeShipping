...
def readPhone():
    ...
    while True:
        ...
        try:
            ...
            des_x = data_dict["des_x"]
            state = data_dict["state"]
            cancel = data_dict["cancel"]
            return des_x, state, cancel
        except:
            pass
    conn.close()
...
"""MAIN LOOP"""
while mode == "selector" && cancel == false:
    readphone()
    pos_x = horizontalPosition(val_old)
    #Assuming read phone outputdes_x, des_y, state, cancel
]
    if state == "idle":
        #Do nothing
        PWM_x = 0
        ...
        homingFunction();
        if homed() == false:
            state = "error_not_homed"
            break;

        while checkEnd() == false:
            PWM_x = calculate_pwm_x(des_x, pos_x)
            writeMotors(PWM_x, checkWatchDog(LS3, LS4))
        state == "finished"
    ...
    elif state == "error_not_homed"
        print("error not homed")
        homingFunction() #Keep trying homing until it's actually homed
        if state == "selector"
            state =="idle"
